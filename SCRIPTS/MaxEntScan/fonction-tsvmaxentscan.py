...
with open(sys.argv[1]) as f: # ouvre le fichier avec les jonction 
		for line in f:
			...
			if lig[0]=="Segment":
				...
				seq=di[lig[0]] #on recupere la séqunece du chromosome segment  qui correspont a cette ligne 
				seq=seq.seq
				seq=str(seq)

				pos5=int(lig[1])-1 # position jontion 5'  le -1 car python conte a partir de zero 
